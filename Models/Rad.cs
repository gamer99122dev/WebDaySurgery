...
{
    /// <summary>
    /// 放射科報告一份 (DB_ADM..AdmRqtrcpRWTbl 表頭 + AdmResultTextTbl 內文)，CXR／KUB 同一個格式。
    /// Report／Impression 在 AdmResultTextTbl 是分開的兩列 (chSegCod 01／02)，這裡併成一筆
    /// </summary>
    public record Rad
    {
        ...
        // 兩個都是 chTxt，靠 chSegCod 分：01 Report、02 Impression。表頭查完才另外查，所以開 set
        public string Report { get; set; } = "";       // chTxt (chSegCod 01)  Report
        public string Impression { get; set; } = "";   // chTxt (chSegCod 02)  Impression
    }
}
